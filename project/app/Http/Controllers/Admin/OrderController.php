...
class OrderController extends AdminBaseController
{
    ...
    //*** JSON Request
    public function refunddatatables(Request $request)
    {
        if ($request->ajax()) {
            $datas = ProductReturnPolicy::with(['product', 'order'])->get();
            return Datatables::of($datas)
                ...
                ->addColumn('action', function ($data) {
                    return '<div class="godropdown"><button class="go-dropdown-toggle"> ' . __("Actions") . '<i class="fas fa-chevron-down"></i></button><div class="action-list"><a data-href="" class="edit" data-toggle="modal" data-target="#editModal" data-id="'.$data['id'].'"> <i class="fas fa-edit"></i> ' . __("Edit") . '</a></a></div></div>';


//                    return '<div class="godropdown"><button class="go-dropdown-toggle"> ' . __("Actions") . '<i class="fas fa-chevron-down"></i></button><div class="action-list"><a data-href="' . route('admin-order-refund-edit', $data->id) . '" class="edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i> ' . __("Edit") . '</a></a></div></div>';
                })
    ...
    public function RefundOrdershow(){

        return view('admin.OrderRefund.orderrefundview');
    }

    public function OrderRefundEdit(Request $request)
    {
         $id=$request->id;
        $data = ProductReturnPolicy::findOrFail($id);
        $data->load('order','product');
        return response()->json($data);
    }

    public function OrderRefundUpdate(Request $request)
    {
        $id = $request->orderrefund_id;
        $orderRefundUpdate = ProductReturnPolicy::findOrFail($id);
        $orderRefundUpdate->refund_amount = $request->amount;
        $orderRefundUpdate->status = $request->status;

        $vendor = $orderRefundUpdate->product->user;
        $user = $orderRefundUpdate->user;
        //dd('vendor', $vendor, 'user', $user);


        // Sending Email To User
        $to = $user->email;
        $user=$user->name ;
        $subject = 'Refund Product.';
//        $msg = "Dear " . $user->name . "<br><br>" . "Your Product has been Refund.";
        $view = view('frontend.mail2.refund_order_email', compact('user'))->render();
...
//            'body' => $msg,
        ...
        $orderRefundUpdate->update();

        if($orderRefundUpdate->status =='Approve'){
            $order = Order::where('id',$orderRefundUpdate->order_id)->first();
            $order->status = 'declined';
            $order->update();
        }

        if($orderRefundUpdate->status =='Cancel'){
            $order = Order::where('id',$orderRefundUpdate->order_id)->first();
            $order->status = 'completed';
            $order->update();
        }

        return response()->json(['success'=>'Refund Status Successfully Update']);
    }
    ...
    public function emailsub(Request $request)
    {
        ...
        {
            $msg=$request->message;
            $view = view('frontend.mail2.sub_email', compact('msg'))->render();
