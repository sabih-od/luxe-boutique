...
class ProductController extends AdminBaseController
{
    ...
    {
        if ($request->type == 'all') {
//            $datas = Product::whereProductType('normal')->where('category_id', '<>', '1')->latest('id')->get();
            $datas = Product::whereProductType('normal')->where('is_printify', '0')->latest('id')->get();
    ...
    public function create($slug)
    {
        ...
        if ($slug == 'physical') {
            ...
            return view('admin.product.create.merchandise', compact('cats', 'sign'));
        }
    }
    ...
    public function edit($id)
    {
        ...
        elseif ($data->type == 'Merchandise')
            return view('admin.product.edit.merchandise', compact('cats', 'data', 'sign'));
        else
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($data->type == "Physical") {
            ...
            if (empty($request->stock_check)) {
                ...
                if (in_array(null, $request->size) || in_array(null, $request->size_qty) || in_array(null, $request->size_price)) {
                    ...
                    $input['stock_check'] = 1;
                    $input['color'] = !empty($request->color) ? implode(',', $request->color) : null;
                    $input['size'] = implode(',', $request->size);
                    $input['size_qty'] = implode(',', $request->size_qty);
                    $size_prices = $request->size_price;
                    $s_price = array();
        ...
        //Product Discount
        if (!is_null($request->previous_price)) {
            $input['price'] = $request->previous_price;
            $input['previous_price'] = $request->price;
        ...
        $input['sale_start_date'] = $request->sale_start_date;
        $input['sale_end_date'] = $request->sale_end_date;
        // store filtering attributes for physical product
        $attrArr = [];
