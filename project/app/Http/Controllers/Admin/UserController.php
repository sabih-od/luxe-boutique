...
class UserController extends AdminBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $view = view('frontend.mail2.store_email', compact('user_name','type'))->render();
        $data = [
            'to' => $data->email,
//            'type' => "new_registration",
//            'cname' => $data->name,
...
// OTHER SECTION ENDS
    ...
    public function admin_sent_to_email_vendor(Request $request){

        $valid=Validator::make($request->all(),[

            "vendor_message"=>"required",
            "vendor_subject"=>"required",
        ]);

        if(!$valid->passes()){

            return response()->json(['error'=>$valid->errors()->toArray()]);
        }

        $to = $request->vendor_email;
        $message =$request->vendor_message;
        $subject = $request->vendor_subject;
        $view = view('frontend.mail2.admin_sent_to_vendor_email_form', compact('message','subject'))->render();
        ...
        return response()->json(['success'=>'SuccessFully sent to vendor email']);
