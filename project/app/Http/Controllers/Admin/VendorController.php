	    ...
	    return Datatables::of($datas)
            ->addColumn('status', function(User $data) {
	            ...
	            ->addColumn('action', function(User $data) {
	                return '<div class="godropdown"><button class="go-dropdown-toggle"> '.__("Actions").'<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="javascript:;" data-href="' . route('admin-vendor-add-subs',$data->id) . '" class="add-subs" data-toggle="modal" data-target="#ad-subscription-modal"> <i class="fas fa-plus"></i> '.__("Add New Plan").'</a><a href="javascript:;" data-href="' . route('admin-vendor-verify',$data->id) . '" class="verify" data-toggle="modal" data-target="#verify-modal"> <i class="fas fa-question"></i> '.__("Ask For Verification").'</a><a href="' . route('admin-vendor-show',$data->id) . '" > <i class="fas fa-eye"></i> '.__("Details").'</a> <a data-href="' . route('admin-vendor-package',$data->id) . '" class="edit" id="btn_vendor_packages" data-toggle="modal" data-target="#modal1"> <i class="fa fa-th-large"></i> '.__("Vendor Packages").'</a> <a data-href="' . route('admin-vendor-edit',$data->id) . '" class="edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i> '.__("Edit").'</a><a href="javascript:;" class="send" data-email="'. $data->email .'" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i> '.__("Send Email").'</a><a href="javascript:;" data-href="' . route('admin-vendor-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> '.__("Delete").'</a></div></div>';
	            })
	            ->rawColumns(['status','action'])
	            ->toJson(); //--- Returning Json Data To Client Side

        // Secret Login
//        <a href="' . route('admin-vendor-secret',$data->id) . '" > <i class="fas fa-user"></i> '.__("Secret Login").'</a>
    }
    ...
    {
        $subs = UserSubscription::where('user_id', $id)->get();

        return view('admin.vendor.package',compact('subs'));
    }
    ...
    public function verifySubmit(Request $request, $id)
    {
        ...
        $user->verifies()->create(['admin_warning' => 1, 'warning_reason' => $request->details]);
                    ...
                    if($settings->is_smtp == 1)
                    {
                        ...
                        $view = view('frontend.mail2.vendor_verification_email', compact('user_name','type'))->render();
...
//                        'cname' => $user->name,
    ...
    {
        $datas = Withdraw::where('type','=','vendor')->with('user')->latest('id')->get();
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
...
//                return '<a href="' . route('admin-vendor-show', $data->user->id) . '" target="_blank">'. $name .'</a>';
                return '<a href="' . route('admin-vendor-show', $data->user_id) . '" target="_blank">'. $name .'</a>';
            })
    ...
    public function addSubsStore(Request $request,$id)
    {
        ...
        if($settings->is_smtp == 1)
        {
            ...
            $view = view('frontend.mail2.addsubstore_email', compact('user_name','type'))->render();
