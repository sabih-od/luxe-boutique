...
class ProductDetailsController extends FrontBaseController
{
    ...
    public function product(Request $request, $slug)
    {
        ...
        $ratings = $productt->ratings->paginate(5);
        ...
        if ($productt->user_id != 0) {
            $vendors = Product::where('status', '=', 1)
                ->where('user_id', '=', $productt->user_id)
...
//        dd($productt);
        ...
        switch($gs->active_theme){
            ...
            case('1'):
                ...
                break;
            case('2'):
                return view('frontend.theme-2.product', compact('productt', 'curr', 'vendors', 'affilate_user', 'ratings'));
                break;
            case('4'):
                return view('frontend.theme-4.product', compact('productt', 'curr', 'vendors', 'affilate_user', 'ratings'));
                break;
    ...
    public function merchandiseProductForm(Request $request){
//        dd($request->all());
        ...
        if($request->hasFile('images')){
            foreach($request->file('images') as $image){
                $imageName = \PriceHelper::ImageCreateName($image);
                $image->move('assets/images/merchandise-request', $imageName);
                $images[] = $imageName;
            }
        }

        $order = new MerchandiseOrders();
        $order->product_id = $request->product_id;
        $order->customer_name = $request->name;
        $order->customer_email = $request->email;
        $order->customer_phone = $request->phone;
        $order->additional_information = $request->description ?? '';
        $order->notes = $request->additional_note ?? '';
        $order->user_media = json_encode($images);
