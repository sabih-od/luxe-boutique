    ...
    public function catalogdatatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('action', function (Product $data) {
                ...
                $catalog = $ck ? '<a href="javascript:;"> ' . __('Added To Catalog') . '</a>' : '<a href="' . route('vendor-prod-catalog-edit', $data->id) . '"><i class="fas fa-plus"></i> ' . __('Add To Catalog') . '</a>' . '<a href="javascript:;" data-href="' . route('vendor-prod-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a>.$catalog .<a data-href="' . route('vendor-prod-feature', $data->id) . '" class="feature" data-toggle="modal" data-target="#modal2"> <i class="fas fa-star"></i> ' . __("Highlight") . '</a>';
...
//        try {
//            $vendorPaymentDetails = VendorPaymentDetails::where('user_id', Auth::user()->id)->firstOrFail();
//            return view('vendor.product.types');
//        } catch (\Exception $exception){
//            return redirect()->route('vendor-payment')->with('error', 'Please Enter Your Payment Details First');
//        }
    }
    ...
    {
        //        $cats = Category::all();
        $cats = Category::where('status', 1)->where('language_id', 1)->get();
        $sign = $this->curr;
        if ($slug == 'physical') {
            ...
            if ($this->gs->merchandise == 1) {
                return view('vendor.product.create.merchandise', compact('cats', 'sign'));
            } else {
                return back();
            }
        } else {
...
//        dd($request->all());
    ...
    public function store(Request $request)
    {
        ...
        $prod_prices = $user->products()->get(); //get all products of user
//        dd($user, $package, $prods, $prod_prices);
        //        if (Generalsetting::find(1)->verify_product == 1) {
        //            if(!$user->checkStatus())
        //            {
        //                return response()->json(array('errors' => [ 0 => __('You must complete your verfication first.')]));
        //            }
        //        }
        if (empty($package) || is_null($package)) {
            return response()->json(array('errors' => [0 => __('Please select your package first.')]));
        }
        //count the total price of products user has added
        $price = 0;
        ...
        //limit reach email to vendor
        if (($package->allowed_products == $prods) && ($package->allowed_products != 0)) {
            $to = $user->email;
            $from = 'noreply@imausashop.com';
            $subject = "Product Limit Reached";
            $msg = '' . $user->name . "</br>";
            $msg .= "You have reached the maximum product limit allowed on your current subscription plan.
                    Please upgrade your subscription plan to more products.<br>";
            $msg .= "Regards<br>";
            $msg .= "Team IMA USA";
        ...
        if (($prods < $package->allowed_products || $package->allowed_products == 0) && $price < $package->total_price) {
            //--- Validation Section
            $rules = [
                'photo' => 'required',
                'file' => 'mimes:zip'
            ...
            if ($request->type == "Physical") {
                ...
                // Check Brand
                if ($request->product_brand_check == "") {
...
//                dd($request->all());
                ...
                if (empty($request->size_check)) {
                    ...
                    $input['size_all'] = implode(',', $request->size);
                }
//            dd('okay');
                ...
                if (empty($request->color_check)) {
                    ...
                    $input['color'] = implode(',', $request->color_all);
                }
            ...
            // Check If it's merchandise product
            if ($request->type == "Merchandise") {
                $input['phone'] = $request->phone;
                $input['address'] = $request->address;
            }

            // Check Features keyword
            ...
            //--- Redirect Section
            return response()->json(array('errors' => [0 => __('You Can\'t Add More Product. Please Upgrade Your Package.')]));
...
//        $cats = Category::all();
        ...
        elseif ($data->type == 'Merchandise')
            return view('vendor.product.edit.merchandise', compact('cats', 'data', 'sign'));
        else
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->type_check == 1) {
            ...
            if ($file = $request->file('file')) {
                $extensions = ['zip'];
                if (!in_array($file->getClientOriginalExtension(), $extensions)) {
                    return response()->json(array('errors' => ['File format not supported']));
                }
                $name = \PriceHelper::ImageCreateName($file);
                $file->move('assets/files', $name);
                $input['file'] = $name;
        ...
        if ($data->type == "Physical") {
            ...
            // Check Color
            if (empty($request->color_check) || is_null($request->color_check)) {
                $input['color_all'] = null;
            } else {
                $input['color_all'] = implode(',', $request->color_all);
            }
...
//         Check Features keyword
                if (!is_null($request->features) && !is_null($request->colors)) {
                    $input['features'] = implode(',', str_replace(',', ' ', $request->features));
                    $input['colors'] = implode(',', str_replace(',', ' ', $request->colors));
                } else {
                    if (is_null($request->features) || is_null($request->colors)) {
    ...
    {
        $data = Product::findOrFail($id);
        return view('vendor.product.highlight', compact('data'));
    }
    ...
    public function catalogupdate(Request $request, $id)
    {
        ...
        if ($prods < $package->allowed_products || $package->allowed_products == 0) {
            ...
            // Conert Price According to Currency
            // Product Discount
            if (!is_null($request->preivoius_price)) {
                $input['price'] = ($request->preivoius_price / $sign->value);
                $input['previous_price'] = ($request->price / $sign->value);
            } else {
                $input['price'] = ($input['price'] / $sign->value);
                $input['previous_price'] = ($input['previous_price'] / $sign->value);
