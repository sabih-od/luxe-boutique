...
//        Returns only the once that are active for checkout from admin panel
//        DB::enableQueryLog();
        return PaymentGateway::where('checkout', 1)
            ->where(function ($q) use ($curr) {
                $q->where('currency_id', 'like', '%\"'.$curr.'\"%');
                $q->orWhere('currency_id','*');
            })->get();
//        dd(DB::getQueryLog());
    }
    ...
    //checkout pr stripe ka form sow nhi karana tha tbhi yeh function bnaya hai
    public function showFormForCheckout(){
        $show = '';
        $data = $this->keyword == null ? 'other' : $this->keyword;
