...
class User extends Authenticatable
{
    ...
    protected $fillable = ['name', 'photo', 'zip', 'city', 'state', 'country', 'address', 'phone', 'fax', 'email', 'password', 'affilate_code', 'verification_link', 'shop_name', 'owner_name', 'shop_number', 'shop_address', 'reg_number', 'shop_message', 'is_vendor', 'shop_details', 'shop_image', 'shipping_cost', 'date', 'mail_sent', 'email_verified', 'email_token', 'reward', 'verification_code'];
    ...
    public function IsAffilatate()
    {
        return !is_null($this->affilate_code);
    }
    ...
    public function favouriteSellerCount()
    {
        return \App\Models\FavoriteSeller::where('user_id', '=', $this->id)->count();
    }
    ...
    {
        $active_subscription = $this->subscribes()->where('status', 1)->first();
        if (is_null($active_subscription)) {
            return false;
        }

        $now = Carbon::now();
        $created_at = Carbon::parse($active_subscription->expired_at);

        $payout_time = $created_at->addDays((int)$active_subscription->payout_time);
        $length_difference = $payout_time->diffInDays($now);
