...
@section('content')
    ...
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __("Mentor Verifications") }}</h4>
                    <ul class="links">
                        <li>
                            <a href="{{ route('admin.dashboard') }}">{{ __("Dashboard") }} </a>
                        </li>
                        <li><a href="javascript:;">{{ __('Mentor Verifications') }}</a></li>
                        <li>
                            <a href="{{ route('admin-mentor-verifications','all') }}">{{ __("All Verifications") }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
{{--                        </div>--}}
                    ...
                    <div class="mr-table allproduct">
                        ...
                        <div class="table-responsive">
                            <table id="mentortable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                                <thead>
                                <tr>
                                    <th>{{ __("Name") }}</th>
                                    <th>{{ __("Email") }}</th>
    ...
    <script type="text/javascript">
        ...
        (function($) {
            ...
            var table = $('#mentortable').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: '{{ route('admin-mentor-datatables','all') }}',
                columns: [
                    { data: 'name', name: 'name' },
                ...
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        if(data[0] == 0)
                        {
                            ...
                            $('.selected-image .row').html('<h3>{{ __("No Files Found.") }}</h3>');
                        }
                        else {
                            ...
                            for(var k in arr)
                            {
                                $('.selected-image .row').append('<div class="col-sm-6">'+
                                    ...
                                    '<a class="btn btn-info" href="'+'{{asset('assets/files/mentor/documents').'/'}}'+arr[k]+'" download> Download File'+
            ...
            $('.f-btn').on('click',function(e){
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        $('#mentortable').DataTable().ajax.reload();
                    }
                });
            });
