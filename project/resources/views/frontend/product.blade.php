                                                ...
                                                <div class="product-wrapper">
                                                    ...
                                                    <div class="product-info">
                                                        ...
                                                        <div class="product-price">
                                                            <div class="price">
                                                                @if(empty($prod->sale_start_date AND $prod->sale_end_date ) OR $prod->sale_start_date <= \Carbon\Carbon::now()->format('Y-m-d') && $prod->sale_end_date >= \Carbon\Carbon::now()->format('Y-m-d') )
                ...
                <div class="col-12">
                    <div class="products product-style-1 owl-mx-5">
                        <div
                            ...
                            {{--                            @foreach (DB::table('products')->where('category_id', $productt->type)->where('product_type', $productt->product_type)->where('language_id',Session::has('language') ? Session::get('language') : 1)->take(12)->get() as $item)--}}
                            @php
                                $related_products = DB::table('products')->where('status', 1)
                                    ->where('category_id', $productt->category->id)
                                    ->orderBy(DB::raw('RAND()'))
                                    ->take(12)
                                    ->get();
                            ...
                            @foreach ($related_products as $item)
                                <div class="item">
                                    <div class="product type-product">
                                        <div class="product-wrapper">
                                            <div class="product-image">
                                                <a href="{{ is_null($item->affiliate_link) ? route('front.product', $item->slug) : $item->affiliate_link }}"
                                                   class="woocommerce-LoopProduct-link">
                                                    <img class="lazy" data-src="{{ $item->photo ? asset('assets/images/products/'.$item->photo):asset('assets/images/noimage.png')}}"
                                                         alt="Product Image"></a>
                                                @if(empty($item->sale_start_date AND $item->sale_end_date ) OR $item->sale_start_date <= \Carbon\Carbon::now()->format('Y-m-d') && $item->sale_end_date >= \Carbon\Carbon::now()->format('Y-m-d') )

{{--                                                    @if( $item->sale_start_date <= \Carbon\Carbon::now()->format('Y-m-d') && $item->sale_end_date >= \Carbon\Carbon::now()->format('Y-m-d'))--}}
                                                    <div class="on-sale">
                                                        -{{ round((int) App\Models\Product::find($item->id)->offPercentage())}}%
                                                        </div>
                                                    @else
