                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav ms-md-5">
                            ...
                            <li class="nav-item dropdown mega-dropdown">
                                <a class="nav-link dropdown-toggle"
                                   href="{{ route('front.category') }}">{{ __('Shop') }}</a>
                                <ul class="dropdown-menu mega-dropdown-menu">
                                    <li class="mega-container">
                                        <div class="row row-cols-lg-4 row-cols-sm-2 row-cols-1">
                                            @foreach (App\Models\Category::where('language_id',$langg->id)->where('status',1)->where('name', '!=', 'Printify')->take(4)->get() as $category)
                            ...
                            <li class="nav-item dropdown {{ request()->path()=='blog' ? 'active' : '' }}">
                                <a class="nav-link dropdown-toggle"
                                 ...
                                 href="{{ route('front.vendor', 'about') }}">{{ __('About Us') }}</a>
                            </li>
{{--                            <li class="nav-item dropdown {{ request()->path()=='blog' ? 'active' : '' }}">--}}
{{--                                <a class="nav-link dropdown-toggle"--}}
            ...
            <div class="col-xl-5 col-md-3">
                <div class="margin-right-1 d-flex align-items-center justify-content-end h-100">
                    ...
                    <div class="search-view">
                        <a href="#search" class="search-pop top-quantity d-flex align-items-center text-decoration-none">
                        ...
                        @if (Auth::check())
                            <a href="{{ route('user-wishlists') }}" class="cart " title="View Wishlist">
                                <div class="cart-icon"><i class="flaticon-like flat-mini mx-auto text-dark"></i> <span
                                        ...
                                        id="wishlist-count">{{ Auth::user()->wishlistCount() }}</span></div>
                            </a>
                            @if (Auth::user()->is_vendor == 0)
                                <a href="{{ route('user-favorites') }}" class="cart " title="View Favourite Seller">
                                    <div class="cart-icon"><i class="flaticon-shop flat-mini mx-auto text-dark"></i> <span
                                            class="header-cart-count "
                                            id="wishlist-count">{{ Auth::user()->favouriteSellerCount() }}</span></div>
                                </a>
                            @endif
                        @else
...
{{--                            <a href="{{ route('user.login') }}" class="cart " title="View Favourite Seller">--}}
{{--                                <div class="cart-icon"><i class="fa-solid fa-code-compare mx-auto text-dark"></i><span--}}
{{--                                        class="header-cart-count" id="wishlist-count">{{ 0 }}</span></div>--}}
{{--                            </a>--}}
                        @endif
                    </div>
                                                    ...
                                                    @foreach (App\Models\Category::where('language_id',$langg->id)->where('status',1)->get() as $category)
                                                        ...
                                                        <li class="cat-item cat-parent">
                                                            <a href="{{route('front.category', $category->slug)}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                                                               class="category-link" id="cat">{{ $category->name }}
                ...
                <div class="d-flex align-items-center justify-content-end h-100 md-py-10">
                    ...
                    <div class="wishlist-view header-cart-1 ms-2">
                        ...
                        @else
                            <a href="{{ route('user.login') }}" class="cart " title="View Wishlist">
                                <div class="cart-icon"><i class="flaticon-like flat-mini mx-auto text-dark"></i> <span
                                        class="header-cart-count" id="wishlist-count1">{{ 0 }}</span></div>
                            </a>
                        @endif
                    </div>
                    ...
                    <form id="searchForm" class="search-form form-inline search-pill-shape"
                        ...
                        <div class="select-appearance-none categori-container" id="catSelectForm">
                            <select name="category" class="form-control categoris" id="category_select">
                                ...
                                @foreach(DB::table('categories')->where('language_id',$langg->id)->where('status',1)->get() as $data)
                                    <option
                                        value="{{ $data->slug }}" {{ Request::route('category') == $data->slug ? 'selected' : '' }}>
                                        {{ $data->name }}
                                    </option>
                                @endforeach
                            </select>
