        ...
        <div class="row align-items-center">
            <div class="col-lg-4 sm-mx-none">
                <div class="d-flex align-items-center text-general">
                    ...
                    <span class="text-dark"><a href="tel:(616) 888-6116" class="text-dark">{{ $ps->phone }}</a></span>
                </div>
            </div>
            <div class="col-lg-8 ">
                        ...
                        <div class="language-selector nice-select">
                            ...
                            <select name="language" class="language selectors nice">
                                @foreach(DB::table('languages')->get() as $language)
                                    <option
                                        value="{{route('front.language',$language->id)}}" {{ Session::has('language') ? ( Session::get('language') == $language->id ? 'selected' : '' ) : (DB::table('languages')->where('is_default','=',1)->first()->id == $language->id ? 'selected' : '') }} >
                                        {{$language->language}}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="currency-selector nice-select">
                            ...
                            <select name="currency" class="currency selectors nice">
                                @foreach(DB::table('currencies')->get() as $currency)
                                    <option
                                        value="{{route('front.currency',$currency->id)}}" {{ Session::has('currency') ? ( Session::get('currency') == $currency->id ? 'selected' : '' ) : (DB::table('currencies')->where('is_default','=',1)->first()->id == $currency->id ? 'selected' : '') }}>
                                        {{$currency->name}}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @else--}}
                            <div class=" align-items-center text-general sell">
                                <a href="{{ route('vendor.register') }}"
                                   class="sell-btn "> {{ __('Signup as a Vendor') }}</a>
{{--                                <a href="{{ route('user.register') }}?is_affiliate=1" class="sell-btn "> {{ __('Signup as an Affiliate') }}</a>--}}
                            </div>
                        @endif
                    @endif
                </ul>
