                                ...
                                <div id="gallery_09" class="product-slide-thumb">
                                    <div class="owl-carousel four-carousel dot-disable nav-arrow-middle owl-mx-5">
                                        @foreach($productt->galleries as $gal)
                                            <div class="item">
                                                <a class="active"
                                                   href="{{ $gal->product->is_printify ? $gal->photo : asset('assets/images/galleries/'.$gal->photo) }}"
                                                   data-image="{{ $gal->product->is_printify ? $gal->photo : asset('assets/images/galleries/'.$gal->photo) }}"
                                                   data-zoom-image="{{ $gal->product->is_printify ? $gal->photo : asset('assets/images/galleries/'.$gal->photo) }}">
                                                    <img
                                                        src="{{ $gal->product->is_printify ? $gal->photo : asset('assets/images/galleries/'.$gal->photo) }}"
                                                        alt="Thumb Image"/>
                                                </a>
                                            </div>
                                        @endforeach
            ...
            <div class="col-12 col-lg-5 col-md-8">
                <div class="summary entry-summary">
                    <div class="summary-inner">
                        ...
                        <input type="hidden" name="email" value="{{ $productt->user->email }}">
                        ...
                        <div class="pro-details">
                            <div class="pro-info">
                                ...
                                <p class="price">
                                    @if(empty($productt->sale_start_date AND $productt->sale_end_date ) OR $productt->sale_start_date <= \Carbon\Carbon::now()->format('Y-m-d') && $productt->sale_end_date >= \Carbon\Carbon::now()->format('Y-m-d') )
                                    ...
                                    <span class="on-sale"><span>{{ round((int)$productt->offPercentage() )}}</span>% Off</span>
                                          @if($productt->sale_start_date AND $productt->sale_end_date)
                                         <h4>Sale</h4>
                                        <span>Sale Start Date : {{$productt->sale_start_date}} </span>
                                    <br>
                                        <span>Sale End Date : {{$productt->sale_end_date}} </span>
                                    @endif
                                    @else
                                        {{ $productt->showPrice() }}
                                    @endif
                                ...
                                @if($productt->product_type == "affiliate")
                                    ...
                                    <li class="addtocart m-1">
                                        <a href="javascript:;" class="affilate-btn"
                                           data-href="{{ $productt->affiliate_link }}"
                                           target="_blank"> {{ __('Buy Now') }}</a>
                                    </li>
                                @else
                                    @if($productt->emptyStock())
                                        <li class="addtocart m-1">
                        ...
                        <div class="yith-wcwl-add-to-wishlist wishlist-fragment mt-3">
                            ...
                            @else
                                <div class="wishlist-button">
                                    <a class="add_to_wishlist" href="{{ route('user.login') }}" data-bs-toggle="tooltip"
                                       data-bs-placement="top" title="" data-bs-original-title="Add to Wishlist"
                                       aria-label="Add to Wishlist">{{ __('Wishlist') }}</a>
                                </div>
                            @endif
                            <div class="compare-button">
                                <a class="compare button" data-href="{{ route('product.compare.add',$productt->id) }}"
                                   href="javascrit:;" data-bs-toggle="tooltip" data-bs-placement="top" title=""
                ...
                <div class="pro-details-sidebar-item mb-4">
                    ...
                    @if( $productt->user_id  != 0)
                        <li class="{{ $gs->is_contact_seller == 0 ? 'contact_seller' : '' }} cnt-sell">
                            <a href="{{ route('front.vendorshop',str_replace(' ', '-', $productt->user->shop_name)) }}"
                               class="view-stor btn--base">
                    ...
                    @if($gs->is_contact_seller == 1)
                        ...
                        @if(Auth::check())
                            ...
                            @if($productt->user_id != 0)
                                ...
                                <a class="view-stor btn--base" href="javascript:;" data-bs-toggle="modal"
                                   data-bs-target="#vendorform">
                    ...
                    @if($productt->user_id != 0)
                        @if(Auth::check())
                            ...
                            @else
                                <a class="view-stor favorite-prod btn--base" href="javascript:;"
                                ...
                                <ul class="price-summary-list">
                                  ...
                                  <span class="woocommerce-Price-amount amount"><h6>{{ __('Discount') }}</h6>
                                    ...
                                    @foreach($productt->whole_sell_qty as $key => $data1)
                                        <li class="selling-price"><label>{{ $productt->whole_sell_qty[$key] }}+</label>
                                            <span><span class="woocommerce-Price-amount amount">{{ $productt->whole_sell_discount[$key] }}% {{ __('Off') }}
                               </span>
                               </span>
                                        </li>
                                    @endforeach
...
{{--                                            </li>--}}
                                            ...
                                            <li>
                                                <textarea class="form-control textarea border mb-1" name="message"
                                                          id="msg" placeholder="{{ __('Your Message *') }}"
                                                          required=""></textarea>
                                            </li>
                                            ...
                                            <input type="text" name="user_id"
                                               ...
                                               <input type="text"name="vendor_shop_name" value="{{ $productt->user->shop_name }}">

                                        </ul>
                                        <button class="submit-btn bg-danger" id="emlsub"
                                                type="submit">{{ __('Send Messages-test') }}</button>
                                    </form>
                                </div>
                            </div>
                        </div>
