...
@if($ps->deal_of_the_day==1)
    ...
    <div class="full-row bg-light deal-div">
        <div class="swiper dealSwiper">
            <div class="swiper-wrapper">
                @foreach($deals as $deal)
                    @if($deal['discount_date'] && \Carbon\Carbon::parse($deal['discount_date'])->diffInMinutes() >= 1)
                        <div class="swiper-slide">
                            <div class="container">
                                <div class="row offer-product align-items-center">
                                    <div class="col-xl-5 col-lg-7">
                                        <h1 class="down-line-secondary text-dark text-uppercase mb-30">{{ __('Deal') }}
                                            <br> {{ __('of the Day') }}</h1>
                                        <div class="product type-product">
                                            <div class="product-wrapper">
                                                <div class="product-info">
{{--                                                    <ins>{{ $deal->showPrice() }} </ins>--}}

                                                    <h3 class="product-title">{{ $deal['name'] ?? '' }} </h3>

                                                    {{$deal->showPrice()}} <del>{{ $deal->showPreviousPrice() }}</del>
                                                    {{--  <h3 class="product-title">{{ $deal['name'] ?? '' }} / $({{ $deal['previous_price'] ?? '' }})</h3>--}}

{{--                                                    {{ dd($deal) }}--}}
                                                    <div class="product-price">

                                                        <div class="on-sale"><span>{{ round($deal->offPercentage()  )}}</span><span>% off</span>

                                                        </div>
{{--                                                        <div class="on-sale"><span>50</span><span>% off</span>--}}
{{--                                                            <del>{{ $deal->showPreviousPrice() }}</del>--}}
{{--                                                        </div>--}}
                                                    </div>
                                                    <div class="font-fifteen">
                                                        {{--<p>{{ ($deal->features[0] ? $deal->features[0] : $deal->features) ?? '' }}</p>--}}
                                                    </div>
                                                    <div class="time-count time-box text-center my-30 flex-between w-75"
                                                         data-countdown="{{ $deal['discount_date'] ?? '0' }}"></div>
                                    ...
                                    <div class="col-xl-6 col-lg-5 offset-xl-1">
                                        <div class="xs-mt-30">
                                            <img
                                                src="{{ $deal['thumbnail'] ? asset('assets/images/thumbnails/'.$deal['thumbnail']) : asset('assets/images/noimage.png') }}"
                                                alt="">
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        const swiper = new Swiper('.dealSwiper', {
            spaceBetween: 0,
            speed: 1000,
            parallax: true,
            grabCursor: true,
                ...
                <div class="col">
                    <div class="top-collection-tab nav-tab-active-secondary">
                        <ul class="nav nav-pills list-color-general justify-content-center mb-5">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" data-bs-toggle="pill"
                                   href="#pills-featured-two">{{ __('Hot Sale') }}</a>
                            </li>
                        </ul>
