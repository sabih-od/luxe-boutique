                                ...
                                @if($sub->title != 'Free')
                                    <div class="col-lg-6">
                                        ...
                                        <span class="price-digit">{{ __('Free') }}</span>
                                            ...
                                            <sup>{{ $curr->sign }}</sup>
                                            <span
                                                class="price-digit">{{ round($sub->price * $curr->value,2) }}</span><br>
                                            <span class="price-month">{{ $sub->payout_time }} {{ __('Day(s)') }}</span>
                                        </span>
                                                @endif
                                            </div>
...
{{--                                                       class="btn btn-default">{{ __('Current Plan') }}</a>--}}
                                                    <a href="{{route('user-vendor-currentpkg',$sub->id)}}"
                                                       class="btn btn-default">{{ __('Current Plan') }}</a>
                                                    <br>
                                                        @if($user->isSubscriptionActive())
{{--                                                    @if($user->date >= Carbon\Carbon::now()->format('Y-m-d'))--}}
                                                        <small
                                                            class="hover-white">{{ __('Ends on:') }} {{ date('d/m/Y',strtotime($user_subs->expired_at)) }}
                                                    ...
                                                    @else
                                                        <small
                                                            class="hover-white text-danger">{{ __('Expired on:') }} {{ date('d/m/Y',strtotime($user_subs->expired_at)) }}
